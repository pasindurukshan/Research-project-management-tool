...
const Supplier = (props) => (
  <tr>
    ...
    <td> {props.Supplier.Supname} </td> <td> {props.Supplier.Amount} </td>{" "}
    <td> {props.Supplier.Date.substring(0, 10)} </td>{" "}
    <td> {props.Supplier.Contactno} </td>
    <td> {props.Supplier.Email} </td>
    <td>
      <Link to={"/supdate/" + props.Supplier._id}> Edit </Link> |{" "}
      <a
        href=" "
        onClick={() => {
          props.deleteSupplier(props.Supplier._id);
        }}
      >
        Delete{" "}
      </a>{" "}
      ...
      <div className="container">
          ...
          <tbody>
            ...
            {this.state.Supplier.map((props) => (
              <tr key={props.id}>
                ...
                <td> {props.Email} </td>
                <td>
                  <Link to={"/Supplier-Edit/" + props._id}>
                    {" "}
                    <Button data-inline="true" variant="warning btn-sm">
                      {" "}
                      Edit{" "}
                    </Button>{" "}
                    |
                  ...
                  <a
                    href=""
                    onClick={() => {
                      this.deleteSupplier(props._id);
                    }}
                  >
                    <Button data-inline="true" variant="danger btn-sm">
                      {" "}
                      Delete{" "}
                    </Button>{" "}
                  </a>
        ...
        <div style={{ float: "left" }}>
          <Link to="/Supplier-report/">
            <button type="button" class="btn btn-danger" variant="primary">
              {" "}
              Report{" "}
            </button>
          </Link>
